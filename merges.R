...
unknown_sharks<-sharks%>%
  filter(is.na(known))%>%
  select(observer,shark_name,trip_id,sighting_number,sighting_id)
...
########### shark sightings #############
shark_sightings_vars<-c("sighting_id","survey_start","observer", "left_id","right_id","sex","size","scars","localisation","taille_chasse","behaviour","code_of_conduct","avoidance_behaviour","end_encounter","boats_min","boats_max","biopsy","tag","tag_no","prey","prey_tube_number","prey_bio_tube_no","shark_name_known")
